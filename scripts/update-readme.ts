...
import { formatNumber, mToNs } from "tinybench";
...
const vitestTable =
  ...
  ).default.testResults[0]!.assertionResults[0]!.benchmarks[0]!.tasks.map((task) => ({
    name: task.name,
...
const output = md.joinBlocks([
  ...
  `Sorted from fastest to slowest. Ran on ${md.link(`https://github.com/${process.env.GITHUB_REPOSITORY}/actions/runs/${process.env.GITHUB_RUN_ID}`, `GitHub Actions runner ${process.env.RUNNER_OS} ${process.env.RUNNER_ARCH} ${process.env.RUNNER_NAME}`)}.`,
  md.table(
    Object.keys(vitestTable[0]!),
    vitestTable.map((row) => Object.values(row)),
  ),
  md.table(
    Object.keys(tinyBenchTable[0]!),
    tinyBenchTable.map((row) => Object.values(row)),
  ),
  md.disclosure(
    "Older results",
...
await writeFile(README_PATH, output);
